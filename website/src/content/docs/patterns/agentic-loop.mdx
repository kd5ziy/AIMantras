---
title: Agentic Loop
description: Iterative plan → act → observe → refine cycles with explicit exit criteria and budgets.
---
...
- Covers tool-driven loops (act on the environment, read the result, adjust) and evaluator-optimizer loops (generate, critique against criteria, revise)
...
| Pair With | When |
|-----------|------|
| [Criterion-Based Evaluation](/patterns/criterion-based-evaluation/) | Make EVALUATE measurable, not vibes |
| [Recursive Self-Eval](/patterns/recursive-self-eval/) | The critique step in the evaluator-optimizer variant |
| [Graph Orchestration](/patterns/graph-orchestration/) | A worker node that needs iteration inside a larger workflow |
| [Planning Phase](/patterns/planning-phase/) | When the goal itself is ambiguous — the loop refines execution, not requirements |
...
- OBSERVE skipped or fabricated: assuming an action worked instead of checking the actual result
